...
class CreateTWorkDays extends Migration {
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up() {
        Schema::create ( 't_work_days', function (Blueprint $table) {
            // 基本カラム定義
            $table->increments ( 'work_days_id' )->comment ( '店舗ID' );
            ...
            $table->string ( 'start_time', 4 )->nullable ()->comment ( '実績開始時刻' );
            $table->string ( 'end_time', 4 )->nullable ()->comment ( '実績終了時刻' );
            $table->smallInteger ( 'start_time_minute' )->nullable ()->comment ( '実績開始時刻分' );
            $table->smallInteger ( 'end_time_minute' )->nullable ()->comment ( '実績終了時刻分' );
            $table->timestamps ();
            
            // FK
            $table->foreign ( 'shop_id' )->references ( 'shop_id' )->on ( 't_shops' );
